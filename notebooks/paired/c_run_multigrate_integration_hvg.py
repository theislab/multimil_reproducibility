...
def train(seed=None, split=None, run=None, config_name=None, **config):
    ...
    rna_end = config["model"]["setup_params"]["rna_indices_end"]
    ...
    if train_params.get('lr', None) is not None:
    ...
    adata[idx_atac_query, :rna_end].X = 0
    adata[idx_scrna_query, rna_end:].X = 0
    adata[idx_snrna_query, rna_end:].X = 0
