...
t_path = input_files[1].replace('query_', '')
t_path = '/'.join(t_path.split('/')[:-1]) + '.ckpt'

print(f'Train checkpoint {t_path}...')
...
print(f'Query checkpoint {input_files[1]}...')
query_state_dict = torch.load(input_files[1])['state_dict']
